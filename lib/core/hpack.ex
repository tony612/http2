defmodule HTTP2.HPACK do
  ...
  """
  @spec new(non_neg_integer(), atom) :: Table.t()
  def new(max_table_size, index \\ :store)
      when is_integer(max_table_size) and max_table_size >= 0 and index in @valid_header_actions do
    Table.new(max_table_size, index)
  end
  ...
  defp encode_headers([{name, value} | rest], %{index: action} = table, acc)
